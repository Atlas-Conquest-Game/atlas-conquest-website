...
Table with stacked sub-line counts. Every column except Card can be hidden from the **Columns** menu; the choice is remembered in `localStorage`. Each percentage cell shows the raw count beneath it in small muted text so all numbers are verifiable by multiplication. Card rows are clickable (prep for future detail popup). Cards with insufficient data (< 5 games for winrates) show "--" and sort to the bottom regardless of sort direction.
...
Optional column group for opening hand mulligan analysis, off by default — shown with the **Mulligan stats** toggle or individually from the **Columns** menu. `mulligan_stats.json` is only fetched once a mulligan column is visible. Cards never seen in an opening hand show "--" in these columns and sort to the bottom. Each game, players see a number of cards equal to their commander's **intellect** stat (6-10), then keep exactly **3** (going first) or **4** (going second), returning the rest to their deck. Only games with mulligan data are counted (tracking began ~Feb 15 2026).
...
| Keep WR Delta | `winrate_delta` | `mulligan_stats.json` | "N seen" or "low sample" | Colored (green positive, red negative). "--" if < 5 seen. |
...
When a commander is selected via the dropdown, mulligan data switches to `commander_mulligan_stats.json` (lazy-loaded on first selection). All stats are scoped to that commander's mulligan games. The `games` field replaces `mulligan_games` as the denominator context.
...
Optional column group, off by default — shown with the **Feedback stats** toggle or individually from the **Columns** menu. Built from the post-match "did you have fun?" prompt; `feedback_stats.json` is only fetched once a feedback column is visible. Only games where at least one player answered count. See `DATA_MODEL.md` for the scope definitions.

| Column | Sort Key | Source | Sub-line | Style |
|--------|----------|--------|----------|-------|
| Positive / Negative | `fb_any_pos` / `fb_any_neg` | `any_pos_rate` / `any_neg_rate` | "N ratings" | Share of all answers in games where either player played the card |
| Positive (Played) / Negative (Played) | `fb_played_pos` / `fb_played_neg` | `played_*_rate` | "N ratings" | Answers from the player who played the card |
| Positive (Opp) / Negative (Opp) | `fb_opp_pos` / `fb_opp_neg` | `opp_*_rate` | "N ratings" | Answers from the player whose opponent played it |

"--" below 5 ratings (sorted to the bottom). Colored against the period × map baseline `fun_rate`: green when the implied positive rate is > 5pp above baseline, red when > 5pp below. With a commander selected, rows come from `by_commander` (games where that commander's player played the card).
